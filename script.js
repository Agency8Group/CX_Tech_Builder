...
CX_Tech_Builder – 경험을 기술로, 고객을 중심에.`;
    ...
    // Toolbox iframe 로드 상태 확인
    function checkToolboxIframe() {
        const toolboxIframe = document.querySelector('.toolbox-iframe');
        const toolboxOverlay = document.querySelector('.toolbox-overlay');
        
        if (toolboxIframe) {
            // iframe 로드 이벤트 리스너 추가
            toolboxIframe.addEventListener('load', function() {
                console.log('Toolbox iframe 로드 완료');
                if (toolboxOverlay) {
                    toolboxOverlay.classList.remove('show');
                }
            });
            
            // iframe 로드 실패 시 처리
            toolboxIframe.addEventListener('error', function() {
                console.error('Toolbox iframe 로드 실패');
                if (toolboxOverlay) {
                    toolboxOverlay.classList.add('show');
                    const toolboxInfo = toolboxOverlay.querySelector('.toolbox-info');
                    if (toolboxInfo) {
                        toolboxInfo.innerHTML = `
                            <h3>⚠️ Toolbox 로드 실패</h3>
                            <p>현재 Toolbox Enterprise Dashboard에 접근할 수 없습니다.</p>
                            <p>잠시 후 다시 시도해주시거나, 아래 링크로 직접 접속해주세요.</p>
                            <a href="https://agency8group.github.io/ToolBox_Enterprise/" target="_blank" class="btn btn-primary">
            ...
            // 10초 후에도 로드되지 않으면 오버레이 표시
            setTimeout(() => {
                if (toolboxIframe.contentDocument === null) {
                    console.log('Toolbox iframe 로드 타임아웃');
                    if (toolboxOverlay) {
                        toolboxOverlay.classList.add('show');
                        const toolboxInfo = toolboxOverlay.querySelector('.toolbox-info');
                        if (toolboxInfo) {
                            toolboxInfo.innerHTML = `
                                <h3>⏳ Toolbox 로딩 중...</h3>
                                <p>Toolbox Enterprise Dashboard를 로드하고 있습니다.</p>
                                <p>잠시만 기다려주세요.</p>
                                <div class="loading-spinner" style="margin: 20px auto; width: 40px; height: 40px; border: 4px solid #f3f3f3; border-top: 4px solid #D4AF37; border-radius: 50%; animation: spin 1s linear infinite;"></div>
                            `;
                        }
                    }
                }
    ...
    // Toolbox iframe 상태 확인 시작
    setTimeout(checkToolboxIframe, 2000);
 });  